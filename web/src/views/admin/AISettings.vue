  ...
  <div class="ai-settings">
    ...
    <div class="section">
      ...
      <div class="task-panel" v-if="task.running && !showWizard">
        <h4 class="task-panel-title">{{ taskTitle }}</h4>
        <TaskProgress :task="task" />
        
        <ErrorLogList 
          v-if="task.failCount > 0 || !task.running"
          :errors="task.errors" 
          :disabled="starting || task.running"
          @retry-all="retryAllFailed"
          @retry-single="retryCard"
        />

        <div class="task-actions" v-if="task.running">
...
export default {
  name: 'AISettings',
  components: { AIBatchWizard, TaskProgress, ErrorLogList },
  ...
  methods: {
    handleVisibilityChange() {
      if (document.visibilityState === 'visible') {
        this.refreshStats();
        if (!this.eventSource) this.initRealtimeUpdates();
      }
    },
    selectProvider(key) {
    ...
    onModelChange() {
      if (this.modelSelect) this.config.model = this.modelSelect;
    },
    async loadConfig() {
      try {
    ...
    updateTaskState(data) {
      ...
      if (this.task.running && !data.running) {
        // 更新快照，但由于我们要显示“已完成”状态 2 秒，保持本地 running: true
        this.task = { ...this.task, ...data, running: true, current: data.total || this.task.total };
        this.refreshStats();
    ...
    async startTask(type, mode) {
      ...
      try {
        ...
        if (!data.success || data.total === 0) {
          this.showToast(data.message || '没有需要处理的卡片', 'info');
          this.task.running = false;
          this.refreshStats();
        } else {
    ...
    async retryAllFailed() {
      const failedIds = this.task.errors.map(e => e.cardId).filter(id => !!id);
    ...
    async doStartBatchTask(cardIds, types) {
      ...
      try {
        ...
        await aiStartBatchTask({ cardIds, types });
        this.initRealtimeUpdates();
      } catch (e) {
        this.showToast(e.response?.data?.message || '启动失败', 'error');
        this.task.running = false;
    ...
    onWizardClose() {
      ...
      this.refreshStats();
      if (!this.task.running) this.initRealtimeUpdates();
    },
    onWizardStart(taskInfo) {
      this.task = { ...this.task, ...taskInfo, running: true, startTime: Date.now() };
      this.initRealtimeUpdates();
    }
  }
};
...
.task-panel-title { margin: 0 0 16px; font-size: 15px; color: #1e40af; }
.task-actions { margin-top: 16px; display: flex; justify-content: flex-end; }
